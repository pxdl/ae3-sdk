...
function parsedPictures(data: Uint8Array, path: string): ParsedPicture[] {
    if (data.length < 0x10 || data[0] !== 0x54 || data[1] !== 0x49
        ...
        throw new Error(`${path}: not a TIM2 texture`);
    if (data[4] !== 4)
        throw new Error(`${path}: TIM2 version ${data[4]} is unsupported`);
    const format = data[5]!;
    if (format !== 0 && format !== 1)
        throw new Error(`${path}: TIM2 format ${format} is unsupported`);
    const pictureCount = data[6]! | data[7]! << 8;
    if (pictureCount < 1) throw new Error(`${path}: TIM2 contains no pictures`);
    ...
    for (let index = 0; index < pictureCount; index++) {
        ...
        if (totalSize < headerSize || headerSize < 0x30)
            throw new Error(`${path}: picture ${index} has invalid sizes`);
        requireRange(data, offset, totalSize, `${path}: picture ${index}`);
        if (!width || !height)
            throw new Error(`${path}: picture ${index} has invalid dimensions ${width}x${height}`);
        const body = offset + headerSize;
        requireRange(data, body, imageSize + clutSize,
                     `${path}: picture ${index} payload`);
...
function decodePalette(data: Uint8Array, picture: ParsedPicture,
    ...
    if (!bytesPerColor)
        throw new Error(`${path}: unsupported TIM2 CLUT type 0x${picture.clutType.toString(16)}`);
    if (picture.colorCount * bytesPerColor > picture.clutSize)
        throw new Error(`${path}: TIM2 palette exceeds declared CLUT size`);
...
export function decodeTim2(data: Uint8Array, pictureIndex = 0,
    ...
    if (!picture) throw new Error(`${path}: TIM2 picture ${pictureIndex} does not exist`);
    ...
    if (picture.imageType === IDTEX4 || picture.imageType === IDTEX8) {
        if (!picture.clutSize || !picture.colorCount)
            throw new Error(`${path}: indexed TIM2 picture has no palette`);
        ...
        if (indexBytes > picture.imageSize)
            throw new Error(`${path}: TIM2 indices exceed declared image size`);
        ...
        if (pixels * bytesPerPixel > picture.imageSize)
            throw new Error(`${path}: TIM2 pixels exceed declared image size`);
        ...
        throw new Error(`${path}: unsupported TIM2 image type ${picture.imageType}`);
    }
